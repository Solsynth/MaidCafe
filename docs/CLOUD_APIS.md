...
the resulting `daemon.alarm.<kind>` notifications. Cloud heartbeat alarms are
generated by the cloud scheduler and are not configured through this route.

#### `POST /api/daemons/:id/push-notification`

Push a notification into the daemon's workspace (e.g. manual maintenance
announcements). `202` returns the created notification; `204` means the
notification was rejected by the effective preference and was discarded.
...
#### `POST /api/notifications/all/read?workspace_id=`

Mark every notification in the workspace read. `204` on success.

#### `GET /api/notification-topics?workspace_id=`

List built-in topics plus topics observed in the workspace's notification
history. Each item has `topic` and `description`.

#### `GET /api/notification-preferences?workspace_id=`

List the caller's custom preferences. An empty `daemon_id` is the account-wide
topic default; a daemon-specific row overrides that default.

#### `PUT /api/notification-preferences/:topic?workspace_id=`
...
`{"workspace_id":"...","preference":0|1|2}`. `DELETE` on the same route resets
the daemon override. Preference levels are:

| Value | Level | Delivery |
| --- | --- | --- |
| `0` | normal | Store in cloud history and publish push |
| `1` | silent | Store in cloud history without push |
| `2` | reject | Discard before history or push |

The daemon override wins over the account-wide topic default. Preferences are
evaluated by the cloud before every daemon notification is persisted or
published, including heartbeat, alarm, webhook, and custom notifications.
