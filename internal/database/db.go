...
type DB struct { *gorm.DB; sqlDB *sql.DB }

func Open(cfg *config.Config) (*DB, error) {
	if cfg == nil || strings.TrimSpace(cfg.Database.DSN) == "" { return nil, fmt.Errorf("database.dsn is required") }
...
func (db *DB) AutoMigrate() error {
	if db == nil || db.DB == nil { return fmt.Errorf("database is nil") }
	return db.DB.AutoMigrate(&Daemon{}, &DaemonMetric{}, &Notification{})
}
func (db *DB) Close() error {
	if db == nil || db.sqlDB == nil { return nil }
